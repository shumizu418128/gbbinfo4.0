...
- **本番ビルド**: ローカルの `npm run build` は常に `sync:build-cache`（全年 bulk SELECT）を走らせるため `DATABASE_URL` が必須。デプロイは GHA（`.github/workflows/ci.yml`）が `sync:tavily` + SSG 検証を行い、Render（`gbbinfo-preview`）が After CI Checks Pass で Dockerfile フルビルドする。`gbbinfo-jpn` への接続は合図まで行わない。
