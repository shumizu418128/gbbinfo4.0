...
- **本番ビルド**: `npm run build` は常に `sync:build-cache`（全年 4 bulk SELECT）を走らせるため `DATABASE_URL` が必須。`astro build` の generating static routes 段は全開催年（2023〜現行年）の Year 行が snapshot に無いと `Year not found` で失敗する。
