...
- **画像アセット + `/_astro`**: `PUBLIC_ASSET_BASE_URL`（dev/build 双方で必須）。本番ビルドは `build.assetsPrefix` で JS/CSS も同 URL を参照する。CI が `dist/_astro` を Pages へ上げてから Render が HTML を出す。オフラインは `npm run assets:dev`（画像のみ）または `assets:dev:with-astro`。
