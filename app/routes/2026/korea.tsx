import type { Route } from "./+types/korea.js";
import { KoreaContent } from "../../pages/2026/KoreaContent.js";
...
export const loader = async ({ params }: Route.LoaderArgs) => {
  ...
  const participants = await findParticipants(YEAR, null, null, KOREA_ISO_CODE);
...
export const meta = ({ data }: Route.MetaArgs) => {
  const env = data?.env;
  const title = `GBB ${YEAR} ${m.team_korea()}`;
...
export const Korea = () => {
  ...
  return (
    <>
      ...
      <HeroImage yearWithCountry={yearWithCountry} subtitle={m.team_korea()} />
      <KoreaContent participants={participants} locale={locale} />
