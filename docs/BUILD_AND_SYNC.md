...
| `sync:tavily` | あり（upsert） | Supabase `Tavily` テーブル | なし |
...
- **目的**: 本番データの Tavily 検索結果・answer 翻訳を更新する
- **いつ実行するか**: 新規出場者追加後、検索結果を更新したいときなど **手動**
- **`dev` / `build` には含まれない**
...
Render.com（Docker）でも同じ `npm run build` が走る。ビルドステージでの Supabase アクセスは **sync 時の 4 bulk SELECT のみ** が目安。
...
| Tavily 本番データを更新したい | `sync:tavily` → `sync:build-cache` → デプロイ |
